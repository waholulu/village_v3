...
# Validates the Phase 1 "hard" difficulty preset:
# 1. The JSON file loads and has the expected hard-mode key values.
...
func test_hard_preset_loads_with_expected_values() -> void:
	...
	# Spot-check the 7 keys the plan locked in for hard mode.
	assert_eq(balance.starting_wood, 1)
	assert_eq(balance.starting_food, 5)
...
func test_default_preset_unchanged() -> void:
	# Phase 1 is purely additive; default balance must not have drifted.
	var balance := BalanceData.new()
...
func test_hard_preset_triggers_wolf_threat_on_night_1() -> void:
	...
	# Inject a wolf within threat radius (10) of HUT.
	...
	# Drive night 1 directly. resolve_night burns 2 wood from starting=1
...
func test_hard_preset_does_not_lose_immediately() -> void:
	# max_campfire_out_nights = 4 means one forced failure isn't a loss.
	# This guards against accidentally making the preset unplayable.
	var balance := BalanceData.new()
